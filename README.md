...
| <a name="input_autolink_references"></a> [autolink\_references](#input\_autolink\_references) | (Optional) The list of autolink references of the repository (key: key\_prefix, value: target\_url\_template - valid URL and contain `<num>` for the reference number) | `map(string)` | `{}` | no |
